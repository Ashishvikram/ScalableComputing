...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        X = numpy.zeros((self.batch_size, self.captcha_height, self.captcha_width, 1), dtype=numpy.float32) #greyscaling
        ...
        for i in range(self.batch_size):
            ...
            grayImage = cv2.cvtColor(raw_data, cv2.COLOR_BGR2GRAY)
            (thresh, blackAndWhiteImage) = cv2.threshold(grayImage, 200, 255, cv2.THRESH_BINARY) #added by Rohan
            processed_data = numpy.array(blackAndWhiteImage)/255
            reshaped_data = numpy.reshape(processed_data,(self.captcha_height, self.captcha_width,1))
            X[i] = reshaped_data
...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        model = create_model(args.length, len(captcha_symbols), (args.height, args.width, 1))#grey scaling
